...
class DistributedSamplerWrapper(DistributedSampler):
    ...
    def __iter__(self):
        if self.sampler.generator is None:
            self.sampler.generator = torch.Generator() #torch.Generator()是PyTorch中的一个类，用于生成随机数种子。具体来说，该类可以用于控制随机数生成器的状态，从而使得随机数生成过程可重复。
...
def lookup_list(index_list, label_csv):
    ...
    return label_list
"""
dataset_train = AudiosetDataset(data_train, label_csv=label_csv, audio_conf=audio_conf_train, 
                                            use_fbank=False, fbank_dir="/public/home/02566/CYX/ast-master/egs/esc50/data/ESC-50-master/fbank", 
                                            roll_mag_aug=False, load_video=False, mode='train')
"""
class AudiosetDataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None, use_fbank=False, fbank_dir=None, roll_mag_aug=False, load_video=False, mode='train'):
        ...
        if self.noise == True:
            print('now use noise augmentation') #没用到
    ...
    def _roll_mag_aug(self, waveform):
        ...
        idx=np.random.randint(len(waveform)) #范围内生成随机数
        rolled_waveform=np.roll(waveform,idx)  #np.roll(a, shift, axis=None)
        mag = np.random.beta(10, 10) + 0.5  #np.random.beta()是NumPy库中的一个函数，用于生成Beta分布的随机数。
        """np.random.beta(a, b, size=None)
            其中：

            a：分布的形状参数alpha。
            b：分布的形状参数beta。
            size：输出数组的形状。如果未指定，则生成单个随机数。"""
    ...
    def _wav2fbank(self, filename, filename2=None):
        ...
        # mixup
        #如果提供了两个音频文件，则进行mixup操作，即将两个音频文件按照一定比例混合在一起
        else:
    ...
    def __getitem__(self, index):
        ...
        """
        torchaudio.transforms.FrequencyMasking()和torchaudio.transforms.TimeMasking()函数对MFCCs进行频率和时间掩蔽。
        """
        ...
        其中，input表示要进行维度交换的张量，dim0和dim1表示要交换的维度。该函数会返回一个新的张量，
        其维度为input的维度在dim0和dim1上交换后得到的维度。

        假设有一个形状为(1, 128, 1024)的张量x，要去除第1维的维度，则可以使用以下代码：
        y = x.squeeze(0)
        需要注意的是，torch.squeeze()并不会改变张量的数据，只会改变张量的维度。如果要改变张量的数据，可以使用torch.view()或torch.reshape()函数。
        """
