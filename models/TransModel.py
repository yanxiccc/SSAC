...
from models.AudioTransformer import AudioTransformer1
...
class MAE_ACT(nn.Module):

    def __init__(self, config, ntoken):
        super(MAE_ACT, self).__init__()
        ...
        mlp_dim = 3072
        dropout = 0.2
        self.encoder = AudioTransformer1(patch_size,
                                        num_classes,
                                        embed_dim,
                                        depth,
                                        num_heads,
    ...
    def decode(self, encoded_feats, tgt, input_mask=None, target_mask=None, target_padding_mask=None):
        ...
        tgt = tgt.transpose(0, 1) #(caption_length, batch_size)
    ...
    def forward(self, src, tgt, input_mask=None, target_mask=None, target_padding_mask=None):
        # src: spectrogram
        #encoded_feats = torch.load('/public/home/02566/CYX/ACT-main_copy/pretrained_models/my_dict1.pth')
        encoded_feats = self.encode(src)

        ''' f_out = open('/public/home/02566/CYX/ACT-main_copy/1.txt', 'r+')
        b = f_out.read()
        b = int(b) + 1
        f_out.seek(0)
        f_out.truncate()
        ...
        f2 = open('/public/home/02566/CYX/ACT-main_copy/1.txt', 'r')
        c = f2.readline()
        y = torch.load('/public/home/02566/CYX/ACT-main_copy/pretrained_models/dict{}.pth'.format(c))
        #torch.save(src, '/public/home/02566/CYX/ACT-main_copy/pretrained_models/dict2.pth')
        encoded_feats = torch.cat((encoded_feats, y)) #64*126*527'''
